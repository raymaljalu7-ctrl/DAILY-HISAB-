...
class BusinessPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final businessItems = <_BusinessItem>[
      ...
      _BusinessItem(title:'Stock Management',subtitle:'Production stock, shop stock, transfers and other received',icon:Icons.inventory_outlined,page:const StockManagementPage()),
      _BusinessItem(title:'Cash Management',subtitle:'Automatic Cash and Bank balances from business transactions',icon:Icons.account_balance_wallet_outlined,page:const CashManagementPage()),
    ...
    return Scaffold(appBar:AppBar(title:const Text('Business')),body:ListView(padding:const EdgeInsets.all(16),children:[
      Card(child:Padding(padding:const EdgeInsets.all(18),child:Row(children:[const CircleAvatar(radius:25,child:Icon(Icons.business_center_outlined,size:26)),const SizedBox(width:14),Expanded(child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[const Text('Business Operations',style:TextStyle(fontSize:20,fontWeight:FontWeight.bold)),const SizedBox(height:4),Text('Manage sales, production, stock, cash, expenses, salary, capital and history.',style:TextStyle(color:Colors.black54))]))]))),
