...
class _SalesPageState extends State<SalesPage> {
  ...
  Future<void> _loadUserProfile() async {
    ...
    setState(() {
      ...
      if (retailUser && shopId != null && shopId.isNotEmpty) selectedShopId = shopId;
    });
  }
  ...
  double get netAmount { final value = grossAmount - commissionAmount; return value < 0 ? 0 : value; }
  double get paymentReceived { final value = double.tryParse(paymentController.text.trim()); return value == null || value < 0 ? 0 : value; }
  ...
  void _removeRow(int index) { if (rows.length == 1) return; rows.removeAt(index).dispose(); setState(() {}); }
  void _update() => setState(() {});

  Future<void> _saveSale(List<Product> products) async {
    if (selectedShop == null) { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Please select a retail shop.'))); return; }
    if (retailUser && assignedShopId != null && selectedShop!.id != assignedShopId) { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('You can only enter sales for your assigned shop.'))); return; }
    final saleItems = <SaleItem>[];
    for (final row in rows) {
      if (row.productId == null) continue;
      if (row.quantity <= 0) { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Quantity must be greater than zero.'))); return; }
      if (row.unitPrice < 0) { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Unit price cannot be negative.'))); return; }
      Product? product;
      for (final item in products) { if (item.id == row.productId) { product = item; break; } }
    ...
    if (saleItems.isEmpty) { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Add at least one product to the sale.'))); return; }
    ...
    if (paid > net) { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Payment received cannot exceed the net amount.'))); return; }
    final outstanding = SalesService.instance.calculateOutstanding(net, paid);
    final sale = Sale(id: editingSaleId ?? '', date: selectedDate, shopId: selectedShop!.id, shopName: selectedShop!.name, items: saleItems, grossAmount: gross, commission: commission, netAmount: net, paymentReceived: paid, outstanding: outstanding, paymentStatus: SalesService.instance.paymentStatus(net, paid), paymentAccount: paymentAccount);
    ...
    try {
      ...
      editingSaleId = null; editingPaymentReceived = 0; paymentController.clear();
  ...
  Future<void> _showDocumentChoice(Sale sale) async {
    if (!mounted) return;
    final choice = await showDialog<String>(context: context, barrierDismissible: false, builder: (context) => AlertDialog(title: const Text('Sale Saved Successfully'), content: const Text('Would you like to generate a sales document?'), actions: [TextButton(onPressed: () => Navigator.pop(context, 'skip'), child: const Text('Skip')), OutlinedButton.icon(onPressed: () => Navigator.pop(context, 'challan'), icon: const Icon(Icons.local_shipping_outlined), label: const Text('Delivery Challan')), FilledButton.icon(onPressed: () => Navigator.pop(context, 'invoice'), icon: const Icon(Icons.receipt_long_outlined), label: const Text('Sales Invoice'))]));
  ...
  Future<void> _generateSalesDocument(Sale sale, {required bool invoice}) async {
    ...
    try {
      await Printing.layoutPdf(onLayout: (format) async {
        ...
        pdf.addPage(pw.MultiPage(pageFormat: PdfPageFormat.a4, margin: const pw.EdgeInsets.all(32), build: (context) => [
          pw.Row(mainAxisAlignment: pw.MainAxisAlignment.spaceBetween, children: [pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [pw.Text('Bakery', style: pw.TextStyle(fontSize: 24, fontWeight: pw.FontWeight.bold)), pw.SizedBox(height: 4), pw.Text('Surat', style: const pw.TextStyle(fontSize: 11))]), pw.Text(documentTitle, style: pw.TextStyle(fontSize: 20, fontWeight: pw.FontWeight.bold))]),
          pw.SizedBox(height: 20), pw.Divider(), pw.SizedBox(height: 10),
          pw.Row(mainAxisAlignment: pw.MainAxisAlignment.spaceBetween, children: [pw.Text('Date: ${_formatDate(sale.date)}', style: const pw.TextStyle(fontSize: 11)), pw.Text('Document No: ${sale.id.isEmpty ? '-' : sale.id.substring(0, sale.id.length > 8 ? 8 : sale.id.length).toUpperCase()}', style: const pw.TextStyle(fontSize: 11))]),
    ...
    } catch (error) { if (!mounted) return; ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Unable to generate document:\n$error'))); }
  }
  ...
  @override
  Widget build(BuildContext context) => Scaffold(appBar: AppBar(title: const Text('Sales')), body: StreamBuilder<List<Map<String, dynamic>>>(stream: FirestoreService.instance.stream('retailShops'), builder: (context, shopSnapshot) {
  ...
  Widget _buildProductRow(int index, List<Product> products) {
    final row = rows[index];
    return Card(margin: const EdgeInsets.only(bottom: 10), child: Padding(padding: const EdgeInsets.all(10), child: Column(children: [Row(children: [Expanded(child: DropdownButtonFormField<String>(initialValue: row.productId, items: products.map((p) => DropdownMenuItem(value: p.id, child: Text(p.name, overflow: TextOverflow.ellipsis))).toList(), onChanged: (value) { row.productId = value; row.updateAmount(); _update(); }, decoration: const InputDecoration(labelText: 'Product', border: OutlineInputBorder()))), if (rows.length > 1) IconButton(onPressed: () => _removeRow(index), icon: const Icon(Icons.delete_outline))]), const SizedBox(height: 10), Row(children: [Expanded(child: TextField(controller: row.quantityController, keyboardType: const TextInputType.numberWithOptions(decimal: true), onChanged: (_) { row.updateAmount(); _update(); }, decoration: const InputDecoration(labelText: 'Quantity', border: OutlineInputBorder()))), const SizedBox(width: 10), Expanded(child: TextField(controller: row.unitPriceController, keyboardType: const TextInputType.numberWithOptions(decimal: true), onChanged: (_) { row.updateAmount(); _update(); }, decoration: const InputDecoration(labelText: 'Rate', prefixText: 'Rs. ', border: OutlineInputBorder())))]), const SizedBox(height: 8), Align(alignment: Alignment.centerRight, child: Text('Amount: Rs. ${row.amount.toStringAsFixed(2)}', style: const TextStyle(fontWeight: FontWeight.w600)))]));
  }

  Widget _summaryRow(String title, double value, {bool bold = false}) => Padding(padding: const EdgeInsets.symmetric(vertical: 3), child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text(title, style: TextStyle(fontWeight: bold ? FontWeight.bold : FontWeight.normal)), Text('Rs. ${value.toStringAsFixed(2)}', style: TextStyle(fontWeight: bold ? FontWeight.bold : FontWeight.normal))]));

  Widget _buildSalesHistory(List<Product> products, List<RetailShop> shops) => StreamBuilder<List<Sale>>(stream: SalesService.instance.watchSales(), builder: (context, snapshot) {
    if (snapshot.hasError) return Card(child: Padding(padding: const EdgeInsets.all(16), child: Text('Error loading sales:\n${snapshot.error}')));
  ...
  void _startEditingSale(Sale sale, List<Product> products, List<RetailShop> shops) {
    for (final row in rows) row.dispose();
    rows.clear();
    for (final item in sale.items) { final row = _SalesRow(); row.productId = item.productId; row.quantityController.text = item.quantity.toString(); row.unitPriceController.text = item.unitPrice.toStringAsFixed(2); row.updateAmount(); rows.add(row); }
    if (rows.isEmpty) rows.add(_SalesRow());
    final shop = shops.where((item) => item.id == sale.shopId).firstOrNull;
    paymentController.text = sale.paymentReceived.toStringAsFixed(2);
    setState(() { editingSaleId = sale.id; editingPaymentReceived = sale.paymentReceived; selectedDate = sale.date; selectedShopId = sale.shopId; selectedShop = shop; paymentAccount = sale.paymentAccount.isEmpty ? 'Cash' : sale.paymentAccount; });
    WidgetsBinding.instance.addPostFrameCallback((_) { if (mounted) Scrollable.ensureVisible(context); });
  }

  Future<void> _deleteSale(Sale sale) async {
    final confirmed = await showDialog<bool>(context: context, builder: (context) => AlertDialog(title: const Text('Delete Sales Transaction'), content: Text('Delete sale for ${sale.shopName}?'), actions: [TextButton(onPressed: () => Navigator.pop(context, false), child: const Text('Cancel')), FilledButton(onPressed: () => Navigator.pop(context, true), child: const Text('Delete'))]));
    ...
    try {
      ...
      if (!mounted) return;
      setState(() => saving = false);
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(admin ? 'Sales transaction deleted.' : 'Delete request sent to Admin for approval.')));
    } catch (error) { if (!mounted) return; setState(() => saving = false); ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Unable to delete sale:\n$error'))); }
  }
}
...
class _SalesRow {
  ...
  void updateAmount() => amount = quantity * unitPrice;
  void clear() { productId = null; quantityController.clear(); unitPriceController.clear(); amount = 0; }
  void dispose() { quantityController.dispose(); unitPriceController.dispose(); }
}
