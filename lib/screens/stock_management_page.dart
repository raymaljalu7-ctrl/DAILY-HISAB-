...
class _StockManagementPageState extends State<StockManagementPage> {
  ...
  Future<void> _saveTransfer(List<Product> products, List<RetailShop> shops) async {
    if (transferProduct == null || transferShop == null) {
      _snack('Select product and shop.');
      return;
    }
    final qty = _num(transferQty.text);
    if (qty <= 0) {
      _snack('Enter quantity greater than zero.');
      return;
    }
  ...
  Future<void> _saveReceived(List<Product> products, List<RetailShop> shops) async {
    if (receivedProduct == null || receivedShop == null) {
      _snack('Select product and shop.');
      return;
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: StreamBuilder<List<Map<String, dynamic>>>(
        ...
        builder: (context, ps) => StreamBuilder<List<Map<String, dynamic>>>(
          ...
          builder: (context, ss) {
            ...
            Widget content;
            if (tab == 0) {
              content = _overview(products, shops);
            } else if (tab == 1) {
              content = _transfer(products, shops);
            } else {
              content = _received(products, shops);
            }
  ...
  Widget _overview(List<Product> products, List<RetailShop> shops) {
    return StreamBuilder<List<Map<String, dynamic>>>(
      ...
      builder: (context, prodSnap) => StreamBuilder<List<Map<String, dynamic>>>(
        stream: FirestoreService.instance.stream('stockTransfers'),
        builder: (context, transferSnap) => StreamBuilder<List<Map<String, dynamic>>>(
          stream: FirestoreService.instance.stream('stockOtherReceived'),
          builder: (context, receivedSnap) => StreamBuilder<List<Map<String, dynamic>>>(
  ...
  Widget _transfer(List<Product> products, List<RetailShop> shops) => ListView(padding: const EdgeInsets.all(12), children: [
    const Text('Production Unit → Retail Shop', style: TextStyle(fontSize: 19, fontWeight: FontWeight.bold)), const SizedBox(height: 14),
    DropdownButtonFormField<String>(initialValue: transferProduct, items: products.map((p) => DropdownMenuItem(value: p.id, child: Text(p.name))).toList(), onChanged: (v) => setState(() => transferProduct = v), decoration: const InputDecoration(labelText: 'Product')), const SizedBox(height: 12),
    DropdownButtonFormField<String>(initialValue: transferShop, items: shops.map((s) => DropdownMenuItem(value: s.id, child: Text(s.name))).toList(), onChanged: (v) => setState(() => transferShop = v), decoration: const InputDecoration(labelText: 'Retail Shop')), const SizedBox(height: 12),
    TextField(controller: transferQty, keyboardType: const TextInputType.numberWithOptions(decimal: true), decoration: const InputDecoration(labelText: 'Quantity', suffixText: 'Box')), const SizedBox(height: 12),
    TextField(controller: transferNote, decoration: const InputDecoration(labelText: 'Note')), const SizedBox(height: 18),
  ...
  Widget _received(List<Product> products, List<RetailShop> shops) => ListView(padding: const EdgeInsets.all(12), children: [
    const Text('Retail Shop Other Received', style: TextStyle(fontSize: 19, fontWeight: FontWeight.bold)), const SizedBox(height: 14),
    DropdownButtonFormField<String>(initialValue: receivedProduct, items: products.map((p) => DropdownMenuItem(value: p.id, child: Text(p.name))).toList(), onChanged: (v) => setState(() => receivedProduct = v), decoration: const InputDecoration(labelText: 'Product')), const SizedBox(height: 12),
    DropdownButtonFormField<String>(initialValue: receivedShop, items: shops.map((s) => DropdownMenuItem(value: s.id, child: Text(s.name))).toList(), onChanged: (v) => setState(() => receivedShop = v), decoration: const InputDecoration(labelText: 'Retail Shop')), const SizedBox(height: 12),
    TextField(controller: receivedQty, keyboardType: const TextInputType.numberWithOptions(decimal: true), decoration: const InputDecoration(labelText: 'Quantity', suffixText: 'Box')), const SizedBox(height: 12),
    TextField(controller: receivedSource, decoration: const InputDecoration(labelText: 'Source / Supplier')), const SizedBox(height: 18),
