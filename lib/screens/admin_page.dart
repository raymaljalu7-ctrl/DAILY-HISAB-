...
class _AdminPageState extends State<AdminPage> {
  ...
  Future<void> _createUser() async {
    ...
    if (!mounted) return;
    await showDialog<void>(context: context, builder: (ctx) => StatefulBuilder(builder: (ctx, set) {
      return AlertDialog(
        ...
        content: SingleChildScrollView(child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          if (role == 'retail_shop_user') ...[
            ...
            DropdownButtonFormField<String>(initialValue: shopId, decoration: const InputDecoration(labelText: 'Assigned Retail Shop'), items: shops.map((s) => DropdownMenuItem<String>(value: s['id'].toString(), child: Text(s['name']?.toString() ?? 'Shop'))).toList(), onChanged: (v) {
              ...
              set(() { shopId = v; shopName = found?['name']?.toString(); });
            }),
          ],
        ])),
        actions: [
          ...
          FilledButton(onPressed: () async {
            ...
            if (!e.contains('@') || password.text.length < 6) { ScaffoldMessenger.of(ctx).showSnackBar(const SnackBar(content: Text('Enter valid email and password of at least 6 characters.'))); return; }
  ...
  Future<void> _backup() async {
    ...
    } catch (e) { if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Backup failed: $e'))); }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: ListView(padding: const EdgeInsets.all(16), children: [
        Card(child: const ListTile(leading: CircleAvatar(child: Icon(Icons.admin_panel_settings)), title: Text('User Management', style: TextStyle(fontWeight: FontWeight.bold)), subtitle: Text('Create users, assign retail shops, approve and deactivate access.'))),
          ...
          return Column(children: docs.map((doc) {
            ...
            return Card(
              ...
              child: ListTile(
                ...
                subtitle: Text('Role: $role\nStatus: $status$shopText'),
                isThreeLine: true,
                trailing: self ? const Text('YOU') : PopupMenuButton<String>(
                  onSelected: (v) { if (v == 'edit') _editUser(doc.id, p); if (v == 'approve') _setStatus(doc.id, 'approved'); if (v == 'disable') _setStatus(doc.id, 'disabled'); if (v == 'reactivate') _setStatus(doc.id, 'approved'); },
                  itemBuilder: (_) => [
