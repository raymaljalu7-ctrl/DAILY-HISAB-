...
class _AdminPageState extends State<AdminPage> {
  ...
  Future<List<Map<String, dynamic>>> getShops() async {
    final snap = await db.collection('sharedData').doc('dailyHisab')
        .collection('retailShops').get();
    ...
    await showDialog<void>(
      ...
      builder: (dialogContext) => StatefulBuilder(
        builder: (context, setDialog) {
          final shopItems = shops.map((shop) {
            return DropdownMenuItem<String>(
              value: shop['id'].toString(),
              child: Text(shop['name']?.toString() ?? 'Shop'),
            );
          }).toList();
          return AlertDialog(
                    ...
                    DropdownButtonFormField<String>(
                      ...
                      onChanged: (value) {
                        ...
                        for (final shop in shops) {
                          if (shop['id'].toString() == value) {
                            selectedName = shop['name']?.toString();
                            break;
                          }
                        }
                        setDialog(() {
                          shopId = value;
                          shopName = selectedName;
                        });
                      },
                    ),
                  ],
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text(path == null ? 'Backup cancelled.' : 'Backup saved successfully.'),
        ));
      }
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [IconButton(onPressed: createUser, icon: const Icon(Icons.person_add_alt_1))],
      ),
      body: ListView(
        ...
        children: [
          const Card(child: ListTile(
            ...
            subtitle: Text('Create users, assign shops, approve and deactivate access.'),
          )),
          const SizedBox(height: 10),
          Row(children: [
            Expanded(child: FilledButton.icon(
              onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const ChangeRequestsPage())),
              icon: const Icon(Icons.approval), label: const Text('CHANGE APPROVALS'),
            )),
            const SizedBox(width: 10),
            Expanded(child: OutlinedButton.icon(
              onPressed: takeBackup, icon: const Icon(Icons.backup_outlined), label: const Text('TAKE BACKUP'),
            )),
          ]),
