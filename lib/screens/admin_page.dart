...
class _AdminPageState extends State<AdminPage> {
                ...
                children: [
                  ...
                  if (role == 'retail_shop_user') ...[
                    ...
                    DropdownButtonFormField<String>(
                      ...
                      items: shops.map((s) => DropdownMenuItem(value: s['id'].toString(), child: Text(s['name']?.toString() ?? 'Shop'))).toList(),
                      onChanged: (v) {
              ...
              FilledButton(
                onPressed: () async {
                  ...
                  if (!e.contains('@') || password.text.length < 6) {
                    ScaffoldMessenger.of(dialogContext).showSnackBar(const SnackBar(content: Text('Enter a valid email and password of at least 6 characters.')));
                    return;
                  }
                  if (role == 'retail_shop_user' && shopId == null) {
                    ScaffoldMessenger.of(dialogContext).showSnackBar(const SnackBar(content: Text('Select the retail shop.')));
                    return;
                  }
                  try {
                    ...
                    if (dialogContext.mounted) ScaffoldMessenger.of(dialogContext).showSnackBar(SnackBar(content: Text('Unable to create user: $e')));
                  }
                },
            ...
            content: Column(
              ...
              children: [
                ...
                if (role == 'retail_shop_user') ...[
                  const SizedBox(height: 12),
                  DropdownButtonFormField<String>(
                    initialValue: validShop ? shopId : null,
                    decoration: const InputDecoration(labelText: 'Assigned Retail Shop'),
                    items: shops.map((s) => DropdownMenuItem(value: s['id'].toString(), child: Text(s['name']?.toString() ?? 'Shop'))).toList(),
                    onChanged: (v) {
                      Map<String, dynamic>? shop;
  ...
  Future<void> restoreBackup() async {
    try {
      final count = await BackupRestoreService.instance.restoreFromFile();
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(count == 0 ? 'Restore cancelled.' : 'Restore completed: $count records restored.')));
    } catch (e) {
                  ...
                  child: ListTile(
                    ...
                    trailing: self
                        ...
                        : PopupMenuButton<String>(
                            onSelected: (value) {
                              if (value == 'edit') editUser(doc.id, data);
                              if (value == 'approve') setStatus(doc.id, 'approved');
                              if (value == 'reject') setStatus(doc.id, 'disabled');
                              if (value == 'disable') setStatus(doc.id, 'disabled');
                              if (value == 'reactivate') setStatus(doc.id, 'approved');
                            },
                            itemBuilder: (_) => [
