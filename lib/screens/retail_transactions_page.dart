...
class RetailTransactionsPage extends StatefulWidget {
  ...
  @override
  State<RetailTransactionsPage> createState() => _RetailTransactionsPageState();
}
...
class _RetailTransactionsPageState extends State<RetailTransactionsPage> {
  ...
  Future<void> _save(List<Party> parties) async {
    ...
    if (needsParty && selectedParty == null) {
      _message('Please select a party.');
      return;
    }
    if (transactionType == 'Payment' &&
        paymentSubType == 'Expense' &&
  ...
  Widget _balanceCard(List<Map<String, dynamic>> rows) {
    double balance(String accountName) {
      var total = 0.0;
      for (final row in rows) {
        if ((row['account']?.toString() ?? 'Cash') != accountName) continue;
        final amount = (row['amount'] as num?)?.toDouble() ?? 0;
        final type = row['type']?.toString() ?? '';
        if (type == 'Receipt' || type == 'Capital') {
          total += amount;
        } else if (type == 'Payment' || type == 'Commission Payment') {
          total -= amount;
        }
      }
      return total;
    ...
    return Card(
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            const Text('End-of-Day Cash / Bank Balance',
            ...
            Text('Total available: ₹${(cash + bank).toStringAsFixed(2)}',
  ...
  Widget _balanceTile(String title, double amount, IconData icon) {
    return Container(
    ...
    return Card(
      child: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text('Retail Transaction Entry',
                style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
              ...
              onChanged: saving
                  ...
                  : (value) {
                      ...
                      setState(() {
                        ...
                        if (value == 'Receipt') paymentSubType = 'Party';
                        selectedParty = null;
                        selectedPartyId = null;
                      });
                    },
            ...
            TextField(
              ...
              decoration: InputDecoration(
                labelText: transactionType == 'Payment' && paymentSubType == 'Expense'
                    ? 'Expense Details'
                    : 'Description',
            ...
            SizedBox(
              ...
              child: FilledButton.icon(
                onPressed: saving ? null : () => _save(parties),
                icon: saving
                    ? const SizedBox(width: 18, height: 18,
                        child: CircularProgressIndicator(strokeWidth: 2))
                    : const Icon(Icons.save),
                label: Text(saving ? 'SAVING...' : 'SAVE TRANSACTION'),
              ),
            ),
          ],
        ),
  ...
  Widget _history(List<Map<String, dynamic>> rows, List<Party> parties) {
    ...
    return Card(
      child: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text('Today / Recent Transactions',
                style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
            const SizedBox(height: 8),
            if (sorted.isEmpty) const Text('No transactions found.'),
            ...sorted.take(30).map((row) {
              ...
              return ListTile(
                ...
                leading: Icon(type == 'Receipt' ? Icons.arrow_downward : Icons.arrow_upward),
                title: Text('$type • ₹${amount.toStringAsFixed(2)}',
                    style: const TextStyle(fontWeight: FontWeight.bold)),
                subtitle: Text(detail.isEmpty ? (row['description']?.toString() ?? '') : detail),
              );
            }),
          ],
        ),
      ...
      body: StreamBuilder<List<Map<String, dynamic>>>(
        ...
        builder: (context, partySnapshot) {
          if (partySnapshot.hasError) {
            return Center(child: Text('Unable to load parties:\n${partySnapshot.error}'));
          }
